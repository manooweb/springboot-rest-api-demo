...
public class TaskController {
  ...
  public TaskResponse create(
      @PathVariable UUID projectId, @Valid @RequestBody TaskCreateRequest request) {
    Task task =
        taskService.create(
            projectId, request.title(), request.description(), request.dueDate(), request.status());
  ...
  public TaskResponse update(
      @PathVariable UUID taskId, @Valid @RequestBody TaskUpdateRequest request) {
    Task task =
        taskService.update(
            taskId, request.title(), request.description(), request.dueDate(), request.status());
  ...
  public List<TaskResponse> listByProject(
    ...
    return taskService.findByProject(projectId, status).stream().map(TaskResponse::from).toList();
  }
